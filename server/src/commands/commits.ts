...
if count == 1 then
  ...
  if idx then
    if redis.call("hset", KEYS[2], ARGV[3], idx) == 1 then
			redis.call("publish", KEYS[1], \'{ \"id\": \"\' .. ARGV[3] .. \'\", \"timestamp\": \' .. ARGV[1] .. \' }\')
			return "OK"
		else
			return redis.error_reply("[CommitStore lua] error writing commit index")
		end
  else
...
export const CommitStore = {
	put: (client: Redis, channel: string, commit: Commit): Promise<Commit> => {
		return new Promise<Commit>(async (resolve, reject) => {
			...
			try {
				const result = await client.eval(lua, 2, [
					`${channel}:Commit`,
					`${channel}:Commit:Idx`,
					timestamp,
	...
	get: (client: Redis, channel: string, args: {
		...
		return new Promise<Commit[]>(async (resolve, reject) => {
			if (args.id) {
				const idx = await client.hget(`${channel}:Commit:Idx`, args.id);
				if (idx) {
					const index = parseInt(idx, 10);
					try {
						resolve(toCommits('[CommitStore.get]', await client.zrange(`${channel}:Commit`, index, index, 'WITHSCORES')));
					} catch (error) {
						reject(error);
					}
				} else {
